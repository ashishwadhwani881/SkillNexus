...
def create_roadmap(
    ...
    roadmap = Roadmap(
        ...
        is_published=roadmap_data.is_published,
    )
    db.add(roadmap)
    db.commit()
    db.refresh(roadmap)
...
def get_my_roadmaps(
    ...
    """List roadmaps assigned to the current user."""
    # Join with UserRoadmap to only get assigned ones
    ...
    roadmaps = db.query(Roadmap).filter(Roadmap.id.in_(assigned_roadmap_ids)).order_by(Roadmap.created_at.desc()).all()
...
def get_roadmap(
    ...
    # Access Control: If learner, ensure roadmap is assigned
    ...
    user_progress = {p.node_id: {"status": p.status.value, "quiz_passed": p.quiz_passed} for p in progress_records}

...
def update_node_progress(
    ...
    """Update a node's progress status. Security: user must be assigned to the roadmap."""
    ...
    if progress_data.status == "done" and not progress.completed_at:
        ...
        if progress.quiz_passed:
            # Strict mode Bonus points
            award_points(db, current_user, "quiz_bonus", f"Strict Mode Bonus for identifying skills in {node.title}", custom_points=5)
    elif progress_data.status == "in_progress" and not progress.started_at:
